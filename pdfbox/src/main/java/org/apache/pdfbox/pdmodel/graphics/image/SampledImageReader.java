...
final class SampledImageReader
{
    ...
    {
        return getRGBImage(pdImage, null, 1, colorKey, null);
    }
    ...
    public static BufferedImage getRGBImage(PDImage pdImage, Rectangle region, int subsampling,
                                            COSArray colorKey, PDColorSpace targetColorSpace) throws IOException
    {
        if (pdImage.isEmpty())
        {
        ...
        {
            return from1Bit(pdImage, clipped, subsampling, width, height, targetColorSpace);
        }
        ...
        if (bitsPerComponent == 8 && Arrays.equals(decode, defaultDecode) && colorKey == null)
        {
            ...
            return from8bit(pdImage, raster, clipped, subsampling, width, height, targetColorSpace);
        }
        return fromAny(pdImage, raster, colorKey, clipped, subsampling, width, height, targetColorSpace);
    }

    private static BufferedImage from1Bit(PDImage pdImage, Rectangle clipped, final int subsampling,
                                          final int width, final int height, PDColorSpace targetColorSpace) throws IOException
    {
        ...
        try (InputStream iis = pdImage.createInputStream(options))
        {
            ...
            return colorSpace.toRGBImage(raster, targetColorSpace);
        }
    }
                                          ...
                                          final int width, final int height, PDColorSpace targetColorSpace) throws IOException
    {
        ...
        try (InputStream input = pdImage.createInputStream(options))
        {
            ...
            if (startx == 0 && starty == 0 && scanWidth == width && scanHeight == height && currentSubsampling == 1)
            {
                ...
                return pdImage.getColorSpace().toRGBImage(raster, targetColorSpace);
            }
    ...
    private static BufferedImage fromAny(PDImage pdImage, WritableRaster raster, COSArray colorKey, Rectangle clipped,
                                         final int subsampling, final int width, final int height, PDColorSpace targetColorSpace)
            throws IOException
    {
        int currentSubsampling = subsampling;
        final PDColorSpace colorSpace = pdImage.getColorSpace();
        ...
        try (ImageInputStream iis = new MemoryCacheImageInputStream(pdImage.createInputStream(options)))
        {
            ...
            // use the color space to convert the image to RGB
            BufferedImage rgbImage = colorSpace.toRGBImage(raster, targetColorSpace);
